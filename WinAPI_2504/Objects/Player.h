...
//	      궁극기 구현하기 (개쎈 레이저 쏘자) + 체력, 궁극기 게이지바 만들기 + weapon 구현하자 + item 까지 구현되는지확인하자
...
private:
	...
	const int MAX_GUN_NUM = 3;
	const Vector2 FIRE_POS = { center.x,center.y - radius };
	const Vector2 FIRE_LEFT = { center.x - radius, FIRE_POS.y };
	const Vector2 FIRE_RIGHT = { center.x + radius, FIRE_POS.y };
